...
// Se pide replantear la forma de insertar este vector de números en la función "VecInsert", 
// de tal manera que el árbol quede lo más balanceado posible, y con menos número de niveles.
...
void inser_bal(CBinTree* t, int* v,int ini,  int fin) {
    if (ini > fin) { return; }
    int medio = (ini + fin) / 2;
    t->Insert(v[medio]);

    inser_bal(t, v, ini, medio - 1);
    inser_bal(t, v, medio + 1, fin);
